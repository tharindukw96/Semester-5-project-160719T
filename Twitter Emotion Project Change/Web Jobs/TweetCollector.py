...
def analyze(keyword):
    ...
    public_tweets = api.search(keyword,lang='en',tweet_mode='extended',count='100',since=start_date.strftime("%Y-%m-%d"))
    #public_tweets += api.search(keyword,lang='en',count='100')
    #public_tweets += api.search(keyword,lang='en',count='100')

    #Tweets text and Tweet time data collect separately
    time = (datetime.datetime.now() - datetime.timedelta(days=2)).strftime("%Y-%m-%d %H:%M:%S")#datetime.datetime.now().strftime("%m/%d/%Y %H:%M:%S")
