...
def analyze(keyword):
    ...
    start_date = (end_date - datetime.timedelta(days=randint(1,6)))
    #Collect the tweets according to given keyword
    public_tweets = api.search(keyword+" -RT",lang='en',tweet_mode='extended',count='100',since=start_date.strftime("%Y-%m-%d"))
    #public_tweets += api.search(keyword,lang='en',count='100')
    #public_tweets += api.search(keyword,lang='en',count='100')

    #Tweets text and Tweet time data collect separately
    time = datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S") # (datetime.datetime.now() - datetime.timedelta(days=2)).strftime("%Y-%m-%d %H:%M:%S")#datetime.datetime.now().strftime("%m/%d/%Y %H:%M:%S")
...
def loop_10():
    ...
    output = db.execute("Select keyword from key_words order by key_id desc")
...
def loop1_10():
    for i in range(1, 100):
        ...
        keywords = ' OR '.join(items)
        collectTweets(items[0])
        print(i)
        time.sleep(5)
        try:
            loop_10()
